...
def combine_text(record: dict) -> str:
    ...
    Parameters:
    - record: dict, the input record containing 'summary' and 'article'
    Return:
    - str, the combined text without duplicate sentences
    """
    # Step 1: Check if both summary and article are available
    summary = record.get("summary", "")  # Provide default value of "" if summary is None
    article = record.get("article", "")  # Provide default value of "" if article is None


    # Step 2: Combine the content of both fields into a single text variable with a delimiter
    combined_text = " ".join([summary, article]).strip()
...
def create_node(row):
    ...
    node = TextNode(
        ...
        text = record["title"] + "\n\n" + f"Impact Score: {record['impact_score']}" + "\n\n" + f"Sentiment: {record['sentiment']}" + "\n\n" + text,
        metadata={
